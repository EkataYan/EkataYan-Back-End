...
class WeatherService:
    ...
    def forecast(self, location, latitude, longitude, forecast_date):
        ...
        try:
            query = location or f"{latitude},{longitude}"
            response = httpx.get("https://api.weatherapi.com/v1/forecast.json",
                                 params={"key": self.config["WEATHER_API_KEY"], "q": query, "dt": forecast_date.isoformat()},
                                 timeout=httpx.Timeout(15, connect=5), follow_redirects=False)
            response.raise_for_status()
            data = response.json()
            forecast = data["forecast"]["forecastday"][0]
            day, astro = forecast["day"], forecast.get("astro", {})
            return {"location": data["location"]["name"], "date": forecast_date.isoformat(),
                    ...
                    "humidity": day.get("avghumidity"), "wind_kph": day.get("maxwind_kph"),
